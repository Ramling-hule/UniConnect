...
export default function DiscoverView() {
  ...
  // Initialize as empty array to prevent first-render crash
  const [users, setUsers] = useState([]); 
  const [loading, setLoading] = useState(true);

  // 1. Fetch Users (With Safety Check)
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        if (Array.isArray(data)) {
            ...
            console.error("API Error:", data); // Log the real error (likely 401 or 500)
  ...
  const handleConnect = async (receiverId) => {
    // Safety check inside handler too
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className={`p-4 rounded-2xl border flex items-center gap-3 transition-colors ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
         ...
         <input type="text" placeholder="Search..." className={`bg-transparent outline-none w-full text-sm ${isDark ? 'text-white' : 'text-slate-900'}`} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {users?.length === 0 && !loading && (
            <p className="col-span-2 text-center text-slate-400 py-10">No users found to connect with.</p>
        )}
