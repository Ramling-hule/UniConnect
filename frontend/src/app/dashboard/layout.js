...
import { 
  Home, Search, Users, Trophy, Layers, Sun, Moon 
} from 'lucide-react';
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen">
        ...
        {/* --- 1. TOP NAVBAR --- */}
        <nav className={`fixed top-0 left-0 right-0 h-16 border-b z-50 px-4 md:px-8 flex items-center justify-between transition-colors duration-500
            ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-brand-primary flex items-center justify-center text-white font-bold text-lg">U</div>
              <span className={`text-xl font-bold hidden md:block ${isDark ? 'text-white' : 'text-slate-900'}`}>UniConnect</span>
            </div>

            <div className="flex items-center gap-4">
              <button onClick={() => dispatch(toggleTheme())} className={`p-2 rounded-full ${isDark ? 'bg-slate-800 text-yellow-400' : 'bg-slate-100 text-slate-600'}`}>
        ...
        <div className="pt-16 flex max-w-[1600px] mx-auto">
            ...
            <aside className={`hidden md:block w-64 fixed h-[calc(100vh-64px)] overflow-y-auto border-r p-4 transition-colors duration-500
              ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
